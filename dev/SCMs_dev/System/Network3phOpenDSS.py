...
# -*- coding: utf-8 -*-
"""
Modification log @Mingzhi Zhang 
#2023.08.14 Support any OpenDSS based distribution model
...
class Network_3ph():
    ...
    def __init__(self, feeder_name, N_phases=3):
        ...
        # Build distribution system model according to the extracted feeder infos
        # Feeder component info is extracted from OpenDSS in advance
        #!Vslack should be checked to make sure it match the line-to-line voltage
        self.feeder_name = feeder_name
        #self.Vslack = 13.2e3 #slack bus line-to-line voltage: Smoky_hill
        self.Vslack = 4.16e3 #slack bus line-to-line voltage: IEEE123bus
        self.Vslack_ph = self.Vslack/np.sqrt(3) #slack bus phase voltage
        self.N_iter = 10 #Z bus power flow iterations
        self.N_phases = 3
...
#                                   + self.bus_df.iloc[bus_i]['Qc']*1j)*1e3
    ...
    def update_YandZ(self):
        ...
        for trans_index in range(self.N_transformers):
            busA_name = self.transformer_df.iloc[trans_index]['busA'].split('.')[0]
            busB_name = self.transformer_df.iloc[trans_index]['busB'].split('.')[0]
            typeA = self.transformer_df.iloc[trans_index]['typeA']
            typeB = self.transformer_df.iloc[trans_index]['typeB']
            busA = self.bus_df[self.bus_df['name']==busA_name]['number'].values[0]                    
            busB = self.bus_df[self.bus_df['name']==busB_name]['number'].values[0]
            ...
            if 'kV_A' in self.transformer_df.columns and \
                ...
                A_i = np.eye(3)/(complex(self.transformer_df.iloc[trans_index]['kV_A'])
                            /complex(self.transformer_df.iloc[trans_index]['kV_B']))
            else:
                A_i = np.eye(3)
...
#Diagonal elements
    ...
    def setup_network_ieee13(self):
        """
        Set up the network as the unloaded IEEE 13 Bus Test Feeder (Manual approach,
        Not from OPENDSS)
    ...
    def loadDssNetwork(self,ntwkName,updateYZ=True,
        ...
        """
        Loading the distribution system model from csv files that are 
        extractd from OpenDSS in advacne:
        ...
        self.feederLoc = os.path.join(path,'../','inputs','opendss_model')#os.path.join(path,'Data','Networks',
        ...
        # TODO: check 是否需要对这个bus 进行类似的处理160.0 -->160
        ...
        # TODO: Check 去掉这个强制complex 之后会不会对程序有影响: (np.complex 已经过期了)
        ...
        for i in range(2,line_df.shape[1]):
            for j in range(line_df.shape[0]):
                ...
                line_df.iloc[j,i] = np.complex_(complex(line_df.iloc[j,i]))
        ...
        if os.path.exists(self.feederLoc+"_trn_df.csv"):
            ...
            for i in [4,5]: # convert to (exactly) same format
                for j in range(trn_df.shape[0]):
                    trnNo = np.complex_(complex(trn_df.iloc[j,i])) 
                    if i==5 and trnNo==0:
        ...
        # ! added to make sure the 160.0 in bus name is changed to 160 to avoid error, 
        # phase value should be represented as X.1, X.2 and X.3
        trn_df.loc[:,'busA'] = self.identify_and_change_format(trn_df.loc[:,'busA'])
        trn_df.loc[:,'busB'] = self.identify_and_change_format(trn_df.loc[:,'busB'])


        #Create capacitor dataframe
        capacitor_columns = ['name','number','bus','kVln','connect',
                             'Qa','Qb','Qc']
        self.capacitor_df = pd.DataFrame(columns = capacitor_columns)
        ...
        # ! added to make sure the 160.0 in bus name is changed to 160 to avoid error, 
        # phase value should be represented as X.1, X.2 and X.3
        cap_df.loc[:,'busA'] = self.identify_and_change_format(cap_df.loc[:,'bus'])
    ...
    def identify_and_change_format(self,strings):
        
        modified_strings = []
        for string in strings:
            if string.endswith('.0'):
                modified_string = string[:-2]  # Remove the last two characters '.0'
