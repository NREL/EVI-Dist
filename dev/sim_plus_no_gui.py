...
configs["controller"] = control_options[6]
configs["month"] = 9
configs["day_of_week"] = 1
configs["sim_name"] = f"TestRun_{configs['feeder']}_{configs['controller']}_{configs['month']}_{configs['day_of_week']}"

sim_start_time = 6*3600
sim_end_time = 6*3600 + 24*3600

input_file_names = dict()
input_file_names["dss_main"] = EVIDIST_ROOT_PATH + f"/data/opendss_model/{configs['feeder']}/Master.dss"
...
# input_file_names["adoption_data"] = EVIDIST_ROOT_PATH + "/data/adoptions/no_ev_loads.csv"
...
with open(EVIDIST_ROOT_PATH + f"/data/saved_sessions_plus/{configs['sim_name']}_data_test.zip", "wb") as f:  # Use 'wb' mode for writing binary data.
