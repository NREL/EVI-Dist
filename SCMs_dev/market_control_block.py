...
#from System.Assets import NondispatchableAsset_3ph
from System.EnergySystem import EnergySystem
from System.Network3phOpenDSS import Network_3ph
from System.DSS_info_extraction import dssConvert
sys.path.append('../block_classes')
from Controller import Controller # Controller is in block_classes
import pickle
import json
import helics as h
import logging
...
class LPMarketController(Controller):
    def __init__(self, name='greedy', cosim=True, helics_config_path='', timestep_sec=60*5, feeder_name='ieee_34', dss_file_name='Main.dss', horizon_sec=24*60*60, evse_df=[], stationary_storage_df=[]):
        super(LPMarketController, self).__init__(name='greedy', cosim=cosim, helics_config_path=helics_config_path, timestep_sec=timestep_sec, feeder_name=feeder_name, horizon_sec=horizon_sec)
    ...
    def setup_controller(self, prices_export, demand_charge, Pmax_market=1000000, Pmin_market=-100000):
        # call the default controller class to setup the helics config
        super().setup_controller()
...
if __name__ == "__main__":
    scm = LPMarketController(helics_config_path='', cosim=False, dss_file_name='../inputs/opendss_model/ieee34.dss')
    logger.debug('scm object created')
    scm.setup_controller(prices_export=0.07, demand_charge=0.9)#scm.setup_controller()
