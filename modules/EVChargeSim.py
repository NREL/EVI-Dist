...
class EVChargeSim:
    def __init__(self, name, cosim, helics_config_path, timestep_sec, sim_start_time, sim_end_time):
    ...
    def setup_charge_sim(self):
        ...
        if self.co_simulation:
            ...
            h.helicsFederateSetTimeProperty(self.fed, h.helics_property_time_delta, 0.0001) #this is the minimum detectable time difference, not the timestep

            h.helicsFederateEnterExecutingMode(self.fed)
        return

    def run_charge_sim(self):
    ...
    def output_charger_load(self):
        ...
        # print(f'EVChargeSim sent {sum(self.ev_loads.values())} at time {self.time}')
        if self.co_simulation:
...
if __name__ == "__main__":
    sim_start_time = int(sys.argv[2])
    sim_end_time = int(sys.argv[3])
    if len(sys.argv)>1:
        # run twice as often, because this is supposed to be a hardware response (no comms delay)
        timestep_sec = int(np.floor(int(sys.argv[1])/10))
        
    evse = EVChargeSim(name="ev_charge_sim", helics_config_path='', cosim=True, timestep_sec=timestep_sec, sim_start_time=sim_start_time, sim_end_time=sim_end_time)
    
    evse.logger.debug('charge sim object created')
    evse.setup_charge_sim()
    for timestep in range(sim_start_time, sim_end_time, evse.timestep_sec):
        ...
        # print(f"EVChargeSim just completed time: {timestep}")
